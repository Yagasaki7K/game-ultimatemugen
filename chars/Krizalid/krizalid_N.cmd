;-| ＡＩ |-----------------------------------------------------

[Command]
name = "CPU1"
command = D, D, D, D, D, D
time = 1
[Command]
name = "CPU2"
command = D, D, D, D, D, D
time = 1
[Command]
name = "CPU3"
command = D, D, D, D, D, D
time = 1
[Command]
name = "CPU4"
command = D, D, D, D, D, D
time = 1
[Command]
name = "CPU5"
command = D, D, D, D, D, D
time = 1
[Command]
name = "CPU6"
command = D, D, D, D, D, D
time = 1
[Command]
name = "CPU7"
command = D, D, D, D, D, D
time = 1
[Command]
name = "CPU8"
command = D, D, D, D, D, D
time = 1
[Command]
name = "CPU9"
command = D, D, D, D, D, D
time = 1
[Command]
name = "CPU10"
command = D, D, D, D, D, D
time = 1
[Command]
name = "CPU11"
command = D, D, D, D, D, D
time = 1
[Command]
name = "CPU12"
command = D, D, D, D, D, D
time = 1
[Command]
name = "CPU13"
command = D, D, D, D, D, D
time = 1
[Command]
name = "CPU14"
command = D, D, D, D, D, D
time = 1
[Command]
name = "CPU15"
command = D, D, D, D, D, D
time = 1
[Command]
name = "CPU16"
command = D, D, D, D, D, D
time = 1
[Command]
name = "CPU17"
command = D, D, D, D, D, D
time = 1
[Command]
name = "CPU18"
command = D, D, D, D, D, D
time = 1
[Command]
name = "CPU19"
command = D, D, D, D, D, D
time = 1
[Command]
name = "CPU20"
command = D, D, D, D, D, D
time = 1

;-| 超必殺技 |-------------------------------------------------
[Command]
name = "hado_ab"
command = ~D, DF, F, a+b
time = 35

[Command]
name = "deth_xy"
command = ~D, F, D, F, x+y
time = 35

[Command]
name = "deth_x"
command = ~D, F, D, F, x
time = 35

[Command]
name = "deth_y"
command = ~D, F, D, F, y
time = 35

[Command]
name = "end_xy"
command = ~D, B, D, B, x+y
time = 35

[Command]
name = "end_x"
command = ~D, B, D, B, x
time = 35

[Command]
name = "end_y"
command = ~D, B, D, B, y
time = 35

[Command]
name = "hado2_xy"
command = ~D, B, D, B, a+b
time = 35

[Command]
name = "hado2_x"
command = ~D, B, D, B, a
time = 35

[Command]
name = "hado2_x"
command = ~D, B, D, B, a
time = 35

[Command]
name = "hado2_y"
command = ~D, B, D, B, b
time = 35

[Command]
name = "hado2_y"
command = ~D, B, D, B, b
time = 35

;-| 必殺技 |---------------------------------------------------
[Command]
name = "hado_x"
command = ~D, DF, F, x
time = 20

[Command]
name = "hado_y"
command = ~D, DF, F, y
time = 20

[Command]
name = "hado_a"
command = ~D, DF, F, a
time = 20

[Command]
name = "hado_b"
command = ~D, DF, F, b
time = 20

[Command]
name = "syoryu_x"
command =  ~F,D,DF, x
time = 15

[Command]
name = "syoryu_y"
command =  ~F,D,DF, y
time = 15

[Command]
name = "tatumaki_ab"
command = ~D, B, a+b
time = 20

[Command]
name = "tatumaki_a"
command = ~D, B, a
time = 20

[Command]
name = "tatumaki_b"
command = ~D, B, b
time = 20

[Command]
name = "gyakuyoga_x"
command = ~F, D, B, x
time = 20

[Command]
name = "gyakuyoga_x"
command = ~F, DF, D, DB, B, x
time = 25

[Command]
name = "gyakuyoga_y"
command = ~F, D, B, y
time = 20

[Command]
name = "gyakuyoga_y"
command = ~F, DF, D, DB, B, y
time = 25

;-| ２回押し技 |-----------------------------------------------
[Command]
name = "FF"     
command = F, F
time = 10

[Command]
name = "BB"     
command = B, B
time = 10

;-| ２・３個の同時押し技 |-------------------------------------
[Command]
name = "recovery"
command = x+y
time = 1

[Command]
name = "recovery"
command = a+b
time = 1

[Command]
name = "recovery"
command = x+a
time = 1

[Command]
name = "recovery"
command = y+b
time = 1

[Command]
name = "b+y"
command = b+y
time = 1

[Command]
name = "a+x"
command = a+x
time = 1

[Command]
name = "a+y"
command = a+y
time = 1

[Command]
name = "x+b"
command = x+b
time = 1

;-| 方向とボタンで出す技 |-------------------------------------
[command]
name = "fwd_a"
command = /F, a
time = 1

[command]
name = "fwd_b"
command = /F, b
time = 1

[command]
name = "fwd_x"
command = /F, x
time = 1

[command]
name = "back_a"
command = /B, a
time = 1

[command]
name = "back_x"
command = /B, x
time = 1

[Command]
name = "down_a"
command = /$D,a
time = 1

[Command]
name = "down_b"
command = /$D,b
time = 1

[command]
name = "downfwd_a"
command = /DF, a
time = 1

[command]
name = "downfwd_y"
command = /DF, y
time = 1

[Command]
name = "up_a"
command = /$U,a
time = 1

[Command]
name = "up_b"
command = /$U,b
time = 1

[Command]
name = "6s"
command = /F,s
time = 1

;-| ボタン設定（いじらない）|----------------------------------
[Command]
name = "a"
command = a
time = 1

[Command]
name = "b"
command = b
time = 1

[Command]
name = "c"
command = c
time = 1

[Command]
name = "x"
command = x
time = 1

[Command]
name = "y"
command = y
time = 1

[Command]
name = "z"
command = z
time = 1

[Command]
name = "start"
command = s
time = 1

;-| 押しっぱなし設定（いじらない）-----------------------------
[Command]
name = "holdfwd"
command = /$F
time = 1

[Command]
name = "holdback"
command = /$B
time = 1

[Command]
name = "holdup" 
command = /$U
time = 1

[Command]
name = "holddown"
command = /$D
time = 1

[Command]
name = "longjump"
command = D, $U

[Command]
name = "hold_a"
command = /$a
time = 1

[Command]
name = "hold_b"
command = /$b
time = 1

[Command]
name = "hold_c"
command = /$c
time = 1

[Command]
name = "hold_x"
command = /$x
time = 1

[Command]
name = "hold_y"
command = /$y
time = 1

[Command]
name = "hold_z"
command = /$z
time = 1

; 下の記述↓は絶対に消さないでください。
[Statedef -1]
;==============================================================
;AI VarSet
[State -1, AI]
type = VarSet
triggerall = var(20) != 1
trigger1  = command = "CPU1"
trigger2  = command = "CPU2"
trigger3  = command = "CPU3"
trigger4  = command = "CPU4"
trigger5  = command = "CPU5"
trigger6  = command = "CPU6"
trigger7  = command = "CPU7"
trigger8  = command = "CPU8"
trigger9  = command = "CPU9"
trigger10  = command = "CPU10"
trigger11  = command = "CPU11"
trigger12  = command = "CPU12"
trigger13  = command = "CPU13"
trigger14  = command = "CPU14"
trigger15  = command = "CPU15"
trigger16  = command = "CPU16"
trigger17  = command = "CPU17"
trigger18  = command = "CPU18"
trigger19  = command = "CPU19"
trigger20  = command = "CPU20"
v =  20
value = 1

;--------------------------------------------------------------
[State -1, AI_800]
type = ChangeState
value = 800
triggerall = var(20) = 1
triggerall = roundstate = 2 
triggerall = statetype = S
triggerall = P2statetype = S || P2statetype = C
triggerall = P2movetype != A || P2movetype != H
trigger1 = P2bodydist X = (0,15)
trigger1 = random <= 300
trigger1 = ctrl
trigger2 = stateno = 100

;--------------------------------------------------------------
;立ち弱Ｐ
[State -1, AI_200]
type = ChangeState
value = 200
triggerall = var(20) = 1
triggerall = roundstate = 2 
triggerall = statetype = S
triggerall = P2MoveType != A
triggerall = P2statetype = S
trigger1 = P2BodyDist X = (19,23)
trigger1 = random <= 200
trigger1 = ctrl
trigger2 = stateno = 100

;立ち弱Ｋ
[State -1, AI_230]
type = ChangeState
value = 230
triggerall = var(20) = 1
triggerall = roundstate = 2 
triggerall = statetype = S
triggerall = P2statetype = S || P2statetype = C
triggerall = P2MoveType != A
trigger1 = P2BodyDist X = (19,26)
trigger1 = random <= 220
trigger1 = ctrl
trigger2 = stateno = 100

;立ち強Ｋ
[State -1, AI_240]
type = ChangeState
value = 240
triggerall = var(20) = 1
triggerall = roundstate = 2 
triggerall = statetype = S
triggerall = P2statetype = S || P2statetype = C
triggerall = P2MoveType != A
trigger1 = P2BodyDist X = (19,38)
trigger1 = random <= 220
trigger1 = ctrl
trigger2 = stateno = 100
trigger3 = p2bodydist X = (55,180)
trigger3 = random <= 20
trigger3 = ctrl
trigger4 = stateno = 100

;立ち強Ｐ
[State -1, AI_240]
type = ChangeState
value = 210
triggerall = var(20) = 1
triggerall = roundstate = 2 
triggerall = statetype = S
triggerall = P2statetype != L
triggerall = P2MoveType != A
trigger1 = P2bodydist Y = (-150,-70)
trigger1 = P2bodydist X = (19,45)
trigger1 = random <= 210
trigger1 = ctrl
trigger2 = stateno = 100

;--------------------------------------------------------------
;屈み弱Ｐ
[State -1, AI_400]
type = ChangeState
value = 400
triggerall = var(20) = 1
triggerall = roundstate = 2 
triggerall = statetype = C 
triggerall = P2statetype = S || P2statetype = C
triggerall = P2MoveType != A
triggerall = P2BodyDist X <= 20
triggerall = random <= 230
trigger1 = ctrl
trigger2 = (stateno = 400) || (stateno = 430)
trigger2 = (time > 9) || (movecontact && time > 5)
trigger3 = stateno = 100

;屈み弱Ｋ
[State -1, AI_430]
type = ChangeState
value = 430
triggerall = var(20) = 1
triggerall = roundstate = 2 
triggerall = statetype = C
triggerall = P2statetype = S || P2statetype = C
triggerall = P2MoveType != A
triggerall = P2BodyDist X <= 20
triggerall = random <= 230
trigger1 = ctrl
trigger2 = (stateno = 400) || (stateno = 430)
trigger2 = (time > 9) || (movecontact && time > 5)
trigger3 = stateno = 100

;屈み強Ｐ
[State -1, AI_410]
type = ChangeState
value = 410
triggerall = var(20) = 1
triggerall = roundstate = 2 
triggerall = statetype = C
triggerall = P2statetype = S || P2statetype = C
triggerall = P2MoveType != A
triggerall = P2BodyDist X <= 40
triggerall = random <= 230
trigger1 = ctrl
trigger2 = stateno = 100

;屈み強Ｋ
[State -1, AI_440]
type = ChangeState
value = 440
triggerall = var(20) = 1
triggerall = roundstate = 2 
triggerall = statetype = C
triggerall = P2statetype = S || P2statetype = C
triggerall = P2MoveType != A
triggerall = P2BodyDist X = (30,60)
trigger1 = random <= 230
trigger1 = ctrl
trigger2 = stateno = 100

;--------------------------------------------------------------
;立ち弱Ｐ近
[State -1, AI_201]
type = ChangeState
value = 201
triggerall = var(20) = 1
triggerall = roundstate = 2 
triggerall = statetype = S
triggerall = P2statetype = S 
triggerall = P2MoveType != A
trigger1 = random <= 200
trigger1 = P2BodyDist X = (3,18)
trigger1 = ctrl
trigger2 = stateno = 100

;立ち弱Ｋ近
[State -1, AI_231]
type = ChangeState
value = 231
triggerall = var(20) = 1
triggerall = roundstate = 2 
triggerall = statetype = S
triggerall = P2statetype = S || P2statetype = C
triggerall = P2MoveType != A
trigger1 = random <= 200
trigger1 = P2BodyDist X = (3,18)
trigger1 = ctrl
trigger2 = stateno = 100

;立ち強Ｐ近
[State -1, AI_211]
type = ChangeState
value = 211
triggerall = var(20) = 1
triggerall = roundstate = 2 
triggerall = statetype = S
triggerall = P2statetype = S 
triggerall = P2MoveType != A
trigger1 = random <= 210
trigger1 = P2BodyDist X = (3,18)
trigger1 = ctrl
trigger2 = stateno = 100

;立ち強Ｋ近
[State -1, AI_241]
type = ChangeState
value = 241
triggerall = var(20) = 1
triggerall = roundstate = 2 
triggerall = statetype = S
triggerall = P2statetype = S || P2statetype = C
triggerall = P2MoveType != A
trigger1 = random <= 205
trigger1 = P2BodyDist X = (3,18)
trigger1 = ctrl
trigger2 = stateno = 100

;---------------------------------------------------------------------------------------------------------------------------
; カットスピン
[State -1, AI_202]
type = ChangeState
value = 202
triggerall = var(20) = 1
triggerall = statetype = S
triggerall = P2statetype = S || P2statetype = C
triggerall = P2MoveType != A
triggerall = roundstate = 2
triggerall = P2BodyDist X = (5,30) 
triggerall = random <= 200
trigger1 = ctrl
trigger2=movehit
trigger2=stateno=200||stateno=201||stateno=210||stateno=211||stateno=230||stateno=235||stateno=240||stateno=241
trigger3=movehit
trigger3=stateno=400||stateno=410||stateno=430
trigger4=movehit
trigger4=stateno=231&&AnimElem=3
trigger5 = stateno = 100

;---------------------------------------------------------------------------------------------------------------------------
; ライジングダークムーンＸ
[State -1,A]
type = ChangeState
value = 1020
triggerall = var(20) = 1
triggerall = roundstate = 2 
triggerall = statetype = S
triggerall = random <= 100 
trigger1 = P2statetype = A
trigger1 = P2BodyDist X = (5,60) 
trigger1 = P2bodydist Y = (-150,-60)
trigger1 = ctrl
trigger2=movehit
trigger2=P2BodyDist X <= 50
trigger2=stateno=200||stateno=201||stateno=210||stateno=211||stateno=230||stateno=235||stateno=240||stateno=241
trigger3=movehit
trigger3=P2BodyDist X <= 50
trigger3=stateno=400||stateno=410||stateno=430||stateno=202
trigger4=movehit
trigger4=stateno=231&&AnimElem=3
trigger5 = P2MoveType = A 
trigger5 = P2BodyDist X <= 45
trigger5 = ctrl

; ライジングダークムーンＹ
[State -1,A]
type = ChangeState
value = 1023
triggerall = var(20) = 1
triggerall = roundstate = 2 
triggerall = statetype = S
triggerall = random <= 90
trigger1 = P2statetype = A
trigger1 = P2BodyDist X = (10,60) 
trigger1 = P2bodydist Y = (-150,-60)
trigger1 = ctrl
trigger2=movehit
trigger2=P2BodyDist X <= 52
trigger2=stateno=200||stateno=201||stateno=210||stateno=211||stateno=230||stateno=235||stateno=240||stateno=241
trigger3=movehit
trigger3=P2BodyDist X <= 52
trigger3=stateno=400||stateno=410||stateno=430||stateno=202
trigger4=movehit
trigger4=stateno=231&&AnimElem=3
trigger5 = P2MoveType = A 
trigger5 = P2BodyDist X <= 45
trigger5 = ctrl

;---------------------------------------------------------------------------------------------------------------------------
; テュホンレイジＸ
[State -1]
type = ChangeState
value = 1000
triggerall = var(20) = 1
triggerall = roundstate = 2 

triggerall = statetype = S
triggerall = p2statetype != L
triggerall = p2bodydist X = (55,180)
triggerall = random <= 50
trigger1 = ctrl
trigger2=movecontact
trigger2=P2BodyDist X <= 50
trigger2=stateno=200||stateno=201||stateno=210||stateno=211||stateno=230||stateno=235||stateno=240||stateno=241
trigger3=movecontact
trigger3=P2BodyDist X <= 50
trigger3=stateno=400||stateno=410||stateno=430||stateno=202
trigger4=movecontact
trigger4=stateno=231&&AnimElem=3
trigger5 = p2bodydist X > 100 
trigger5 = (P2Movetype = A) || (Enemy,NumProj)
trigger5 = ctrl

; テュホンレイジＹ
[State -1]
type = ChangeState
value = 1005
triggerall = var(20) = 1
triggerall = roundstate = 2
 
triggerall = statetype = S
triggerall = p2statetype != L
triggerall = p2bodydist X = (10,80)
triggerall = random <= 50
trigger1 = ctrl
trigger2=movecontact
trigger2=stateno=200||stateno=201||stateno=210||stateno=211||stateno=230||stateno=235||stateno=240||stateno=241
trigger3=movecontact
trigger3=stateno=400||stateno=410||stateno=430||stateno=202
trigger4=movecontact
trigger4=stateno=231&&AnimElem=3
trigger5 = (P2Movetype = A) || (Enemy,NumProj)
trigger5 = ctrl

;---------------------------------------------------------------------------------------------------------------------------
; リーサルインパクト_Ａ
[State -1]
type = ChangeState
value = 1010
triggerall = var(20) = 1
triggerall = roundstate = 2 
triggerall = statetype = S
triggerall = p2statetype != L
triggerall = P2BodyDist X = (5,60) 
triggerall = random <= 80
trigger1 = ctrl
trigger2=movecontact
trigger2=stateno=200||stateno=201||stateno=210||stateno=211||stateno=230||stateno=235||stateno=240||stateno=241
trigger3=movecontact
trigger3=stateno=400||stateno=410||stateno=430||stateno=202
trigger4=movecontact
trigger4=stateno=231&&AnimElem=3
trigger5 = P2MoveType = A && ctrl
trigger5 = Enemy,NumProj!
trigger6 = ctrl
trigger6 = P2BodyDist X = (10,60) 
trigger6 = P2bodydist Y = (-150,-60)

;---------------------------------------------------------------------------------------------------------------------------
; リーサルインパクト_Ｂ
[State -1]
type = ChangeState
value = 1017
triggerall = var(20) = 1
triggerall = roundstate = 2 
triggerall = statetype = S
triggerall = p2statetype != L
triggerall = P2BodyDist X = (5,60) 
triggerall = random <= 80
trigger1 = ctrl
trigger2=movecontact
trigger2=stateno=200||stateno=201||stateno=210||stateno=211||stateno=230||stateno=235||stateno=240||stateno=241
trigger3=movecontact
trigger3=stateno=400||stateno=410||stateno=430||stateno=202
trigger4=movecontact
trigger4=stateno=231&&AnimElem=3
trigger5 = P2MoveType = A && ctrl
trigger5 = Enemy,NumProj!
trigger6 = ctrl
trigger6 = P2BodyDist X = (10,60) 
trigger6 = P2bodydist Y = (-150,-60)

;---------------------------------------------------------------------------------------------------------------------------
; デスペレイトモーメント
[State -1]
type = ChangeState
value = 1030
triggerall = var(20) = 1
triggerall = roundstate = 2 
triggerall = statetype = S
triggerall = p2statetype != L
triggerall = P2MoveType != A
triggerall = p2bodydist X <= 140
trigger1 = random <= 35 
trigger1 = Enemy,NumProj!
trigger1 = ctrl
trigger2=movecontact
trigger2=stateno=200||stateno=201||stateno=210||stateno=211||stateno=230||stateno=235||stateno=240||stateno=241
trigger3=movecontact
trigger3=stateno=400||stateno=410||stateno=430||stateno=202
trigger4=movecontact
trigger4=stateno=231&&AnimElem=3

;---------------------------------------------------------------------------------------------------------------------------
; サブバーシブ弱・・・
[State -1]
type = ChangeState
value = 1040
triggerall = var(20) = 1
triggerall = roundstate = 2 
triggerall = statetype != L
triggerall = statetype != A
triggerall = p2statetype = S
triggerall = p2bodydist X > 130
trigger1 = random < 8
trigger1 = ctrl
trigger1 = Enemy,NumProj!

; サブバーシブ強・・・
[State -1]
type = ChangeState
value = 1041
triggerall = var(20) = 1
triggerall = roundstate = 2 
triggerall = statetype != A
triggerall = P2StateType != A
triggerall = p2bodydist X <= 65
triggerall = random < 14
trigger1 = ctrl
trigger2=movecontact
trigger2=stateno=200||stateno=201||stateno=210||stateno=211||stateno=230||stateno=235||stateno=240||stateno=241
trigger3=movecontact
trigger3=stateno=400||stateno=410||stateno=430||stateno=202
trigger4=movecontact
trigger4=stateno=231&&AnimElem=3

;---------------------------------------------------------------------------------------------------------------------------
; フィアース・コンフラグレーション
[State -1]
type = ChangeState
value = 3500
triggerall = var(20) = 1
triggerall = power >= 3000 
triggerall = roundstate = 2 
triggerall = statetype = S
triggerall = p2statetype != L
triggerall = p2statetype != A
triggerall = p2bodydist X <= 60
trigger1 = random <= 80 
trigger1 = ctrl
trigger2=movehit
trigger2=stateno=200||stateno=201||stateno=210||stateno=211||stateno=230||stateno=235||stateno=240||stateno=241
trigger3=movehit
trigger3=stateno=400||stateno=410||stateno=430||stateno=202
trigger4=movehit
trigger4=stateno=231&&AnimElem=3

;---------------------------------------------------------------------------------------------------------------------------
; デスペレイト・オーバードライブ・ＬＶ２
[State -1]
type = ChangeState
value = 3100
triggerall = var(20) = 1
triggerall = power >= 2200 
triggerall = roundstate = 2 
triggerall = statetype = S
triggerall = p2statetype != L
triggerall = p2statetype != A
trigger1 = p2bodydist X <= 140
trigger1 = random <= 10
trigger1 = Enemy,NumProj!
trigger1 = ctrl
trigger2=movehit
trigger2=stateno=200||stateno=201||stateno=210||stateno=211||stateno=230||stateno=235||stateno=240||stateno=241
trigger3=movehit
trigger3=stateno=400||stateno=410||stateno=430||stateno=202
trigger4=movehit
trigger4=stateno=231&&AnimElem=3
trigger5 = P2MoveType = A 
trigger5 = p2bodydist X <= 70
trigger5 = random <= 10
trigger5 = ctrl

;---------------------------------------------------------------------------------------------------------------------------
; デスペレイト・オーバードライブ
[State -1]
type = ChangeState
value = 3000
triggerall = var(20) = 1
triggerall = power >= 1200 
triggerall = roundstate = 2 
triggerall = statetype = S
triggerall = p2statetype != L
triggerall = p2statetype != A
trigger1 = p2bodydist X <= 150
trigger1 = random <= 10
trigger1 = Enemy,NumProj!
trigger1 = ctrl
trigger2=movehit
trigger2=stateno=200||stateno=201||stateno=210||stateno=211||stateno=230||stateno=235||stateno=240||stateno=241
trigger3=movehit
trigger3=stateno=400||stateno=410||stateno=430||stateno=202
trigger4=movehit
trigger4=stateno=231&&AnimElem=3
trigger5 = P2MoveType = A 
trigger5 = p2bodydist X <= 70
trigger5 = random <= 10
trigger5 = ctrl

;---------------------------------------------------------------------------------------------------------------------------
; エンドオブヘブン
[State -1]
type = ChangeState
value = 3200
triggerall = var(20) = 1
triggerall = power >= 1200 
triggerall = roundstate = 2 
triggerall = statetype = S
triggerall = p2statetype != L
trigger1 = p2bodydist X <= 60
trigger1 = p2bodydist Y = (-180,-70)
trigger1 = random <= 60 
trigger1 = ctrl
trigger2 = p2bodydist X <= 60
trigger2 = p2bodydist Y = (-180,-70)
trigger2 = P2MoveType = A 
trigger2 = random <= 60
trigger2 = ctrl

;--------------------------------------------------------------
; エンドオブエデン
[State -1]
type = ChangeState
value = 3300
triggerall = var(20) = 1
triggerall = power >= 2100 
triggerall = roundstate = 2 
triggerall = statetype = S
trigger1 = p2bodydist X = [50,160]
trigger1 = random <= 10 
trigger1 =  ctrl
trigger2 = p2bodydist X <= 50
trigger2 = p2bodydist Y = (-180,-60)
trigger2 = random <= 20 
trigger2 = ctrl

;--------------------------------------------------------------
; ブルータル・アンギッシュＸ
[State -1]
type = ChangeState
value = 3400
triggerall = var(20) = 1
triggerall = power >= 1250 
triggerall = roundstate = 2 
triggerall = statetype = S
triggerall = p2statetype != L
triggerall = p2statetype != C
trigger1 = p2bodydist X >= 90
trigger1 = random <= 5
trigger1 = (P2Movetype = A) || (Enemy,NumProj)
trigger1 = ctrl

; ブルータル・アンギッシュＹ
[State -1]
type = ChangeState
value = 3405
triggerall = var(20) = 1
triggerall = power >= 1250 
triggerall = roundstate = 2 
triggerall = statetype = S
triggerall = p2statetype != L
triggerall = p2statetype != C
trigger1 = p2bodydist X >= 90
trigger1 = random <= 5
trigger1 = (P2Movetype = A) || (Enemy,NumProj)
trigger1 = ctrl

; ブルータル・アンギッシュＭＡＸ
[State -1]
type = ChangeState
value = 3410
triggerall = var(20) = 1
triggerall = power >= 2100
triggerall = roundstate = 2 
triggerall = statetype = S
triggerall = p2statetype != L
triggerall = p2statetype != C
trigger1 = p2bodydist X >= 90
trigger1 = random <= 5
trigger1 = (P2Movetype = A) || (Enemy,NumProj)
trigger1 = ctrl

;--------------------------------------------------------------
;挑発
[State -1, Taunt]
type = ChangeState
value = 195
triggerall = var(20) = 1
triggerall = roundstate = 2 
triggerall = statetype = S
triggerall = p2statetype = L
trigger1 = random < 5
trigger1 = ctrl

;---------------------------------------------------------------------------------------------------------------------------
[State -1, 空中強Ｐ]
type = ChangeState
value = 610
triggerall = var(20) = 1
triggerall = roundstate = 2 
triggerall = statetype = A
triggerall = p2statetype != L
trigger1 = p2bodydist X = (20,65)
trigger1 = random <= 100 
trigger1 = ctrl

[State -1, 空中強Ｋ]
type = ChangeState
value = 640
triggerall = var(20) = 1
triggerall = roundstate = 2 
triggerall = statetype = A
triggerall = p2statetype != L
trigger1 = p2bodydist X = (20,70)
trigger1 = random <= 90 
trigger1 = ctrl

[State -1, 空中弱Ｐ]
type = ChangeState
value = 600
triggerall = var(20) = 1
triggerall = roundstate = 2 
triggerall = statetype = A
triggerall = p2statetype != L
trigger1 = p2bodydist X = (5,40)
trigger1 = random <= 90 
trigger1 = ctrl

[State -1, 空中弱Ｋ]
type = ChangeState
value = 630
triggerall = var(20) = 1
triggerall = roundstate = 2 
triggerall = statetype = A
triggerall = p2statetype != L
trigger1 = p2bodydist X = (5,40)
trigger1 = random <= 90 
trigger1 = ctrl

[State -1, 空中ふっとばし]
type = ChangeState
value = 650
triggerall = var(20) = 1
triggerall = roundstate = 2 
triggerall = statetype = A
triggerall = p2statetype != L
trigger1 = p2bodydist X = (20,70)
trigger1 = random <= 100 
trigger1 = ctrl

;---------------------------------------------------------------------------------------------------------------------------
; デモンランディングＡ
[State -1]
type = ChangeState
value = 1050
triggerall = Facing != enemynear,Facing
triggerall = var(20) = 1
triggerall = roundstate = 2 
triggerall = random <= 70
trigger1 = p2bodydist X = (20,80) 
trigger1 = Pos Y <= -60
trigger1 = statetype = A
trigger1 = ctrl
trigger2=movehit
trigger2=stateno=600||stateno=610||stateno=630||stateno=631||stateno=640

; デモンランディングＢ
[State -1]
type = ChangeState
value = 1055
triggerall = Facing != enemynear,Facing
triggerall = var(20) = 1 
triggerall = roundstate = 2 
triggerall = random <= 70
trigger1 = p2bodydist X = (30,100) 
trigger1 = Pos Y <= -60
trigger1 = statetype = A
trigger1 = ctrl
trigger2=movehit
trigger2=stateno=600||stateno=610||stateno=630||stateno=631||stateno=640

;---------------------------------------------------------------------------------------------------------------------------
; 緊急回避
[State -1]
type = ChangeState
value = 850
triggerall = command = "a+x" || command = "z"
triggerall = command != "holdback"
triggerall = stateno != 100
trigger1 = ctrl && statetype != A

; 緊急回避
[State -1]
type = ChangeState
value = 860
triggerall = command = "a+x"|| command = "z"
triggerall = command = "holdback"
trigger1 = ctrl && statetype != A
trigger2 = stateno = 100

; 緊急回避
[State -1]
type = ChangeState
value = 851
triggerall = command = "a+x" || command = "z"
trigger1 = ctrl && statetype != A
trigger2 = stateno = 100

;--------------------------------------------------------------
; ガードキャンセル緊急回避
[State -1]
type = ChangeState
value = ifelse(command != "holdback",830,835)
triggerall = command = "a+x" || command = "z"
triggerall = power >= 1000
trigger1 = stateno = [150,153]

;--------------------------------------------------------------
; ガードキャンセルふっとばし攻撃
[State -1]
type = ChangeState
value = 840
triggerall = command = "b+y"|| command = "c"
triggerall = power >= 1000
trigger1 = stateno = [150,153]

;--------------------------------------------------------------
; ダッシュ
[State -1, Run Fwd]
type = ChangeState
value = 100
trigger1 = command = "FF"
trigger1 = statetype = S
trigger1 = Stateno != 100
trigger1 = Stateno != 101
trigger1 = ctrl

;--------------------------------------------------------------
; バックステップ
[State -1, Run Back]
type = ChangeState
value = 105
trigger1 = command = "BB"
trigger1 = statetype = S
trigger1 = ctrl

;---------------------------------------------------------------------------------------------------------------------------
; フィアース・コンフラグレーション
[State -1]
type = ChangeState
value = 3500
triggerall = power >= 3000 
triggerall = command = "hado_ab" 
trigger1 = statetype != A
trigger1 = ctrl
trigger2=movecontact
trigger2=stateno=200||stateno=201||stateno=210||stateno=211||stateno=230||stateno=231||stateno=235||stateno=240||stateno=241
trigger3=movecontact
trigger3=stateno=400||stateno=410||stateno=430||stateno=202
trigger4 = stateno = 250

;---------------------------------------------------------------------------------------------------------------------------
; デスペレイト・オーバードライブ・レベル２
[State -1]
type = ChangeState
value = 3100
triggerall = power >= 2000 
triggerall = command = "deth_xy"
trigger1 = statetype != A
trigger1 = ctrl
trigger2=movecontact
trigger2=stateno=200||stateno=201||stateno=210||stateno=211||stateno=230||stateno=231||stateno=235||stateno=240||stateno=241
trigger3=movecontact
trigger3=stateno=400||stateno=410||stateno=430||stateno=202
trigger4 = stateno = 250

;---------------------------------------------------------------------------------------------------------------------------
; デスペレイト・オーバードライブＸ
[State -1]
type = ChangeState
value = 3000
triggerall = power >= 1000 
triggerall = command = "deth_x"
trigger1 = statetype != A
trigger1 = ctrl
trigger2=movecontact
trigger2=stateno=200||stateno=201||stateno=210||stateno=211||stateno=230||stateno=231||stateno=235||stateno=240||stateno=241
trigger3=movecontact
trigger3=stateno=400||stateno=410||stateno=430||stateno=202
trigger4 = stateno = 250

;---------------------------------------------------------------------------------------------------------------------------
; デスペレイト・オーバードライブＹ
[State -1]
type = ChangeState
value = 3001
triggerall = power >= 1000 
triggerall = command = "deth_y"
trigger1 = statetype != A
trigger1 = ctrl
trigger2=movecontact
trigger2=stateno=200||stateno=201||stateno=210||stateno=211||stateno=230||stateno=231||stateno=235||stateno=240||stateno=241
trigger3=movecontact
trigger3=stateno=400||stateno=410||stateno=430||stateno=202
trigger4 = stateno = 250

;---------------------------------------------------------------------------------------------------------------------------
; エンドオブエデン
[State -1]
type = ChangeState
value = 3300
triggerall = power >= 2000 
triggerall = command = "end_xy"
trigger1 = statetype != A
trigger1 = ctrl
trigger2=movecontact
trigger2=stateno=200||stateno=201||stateno=210||stateno=211||stateno=230||stateno=231||stateno=235||stateno=240||stateno=241
trigger3=movecontact
trigger3=stateno=400||stateno=410||stateno=430||stateno=202
trigger4 = stateno = 250

;---------------------------------------------------------------------------------------------------------------------------
; エンドオブヘブン
[State -1]
type = ChangeState
value = 3200
triggerall = power >= 1000 
triggerall = command = "end_x" || command = "end_y" 
trigger1 = statetype != A
trigger1 = ctrl
trigger2=movecontact
trigger2=stateno=200||stateno=201||stateno=210||stateno=211||stateno=230||stateno=231||stateno=235||stateno=240||stateno=241
trigger3=movecontact
trigger3=stateno=400||stateno=410||stateno=430||stateno=202
trigger4 = stateno = 250

;---------------------------------------------------------------------------------------------------------------------------
; ブルータル・アンギッシュＭＡＸ
[State -1]
type = ChangeState
value = 3410
triggerall = power >= 2000 
triggerall = command = "hado2_xy" 
trigger1 = statetype != A
trigger1 = ctrl
trigger2=movecontact
trigger2=stateno=200||stateno=201||stateno=210||stateno=211||stateno=230||stateno=231||stateno=235||stateno=240||stateno=241
trigger3=movecontact
trigger3=stateno=400||stateno=410||stateno=430||stateno=202
trigger4 = stateno = 250

;---------------------------------------------------------------------------------------------------------------------------
; ブルータル・アンギッシュＸ
[State -1]
type = ChangeState
value = 3400
triggerall = power >= 1000 
triggerall = command = "hado2_x" 
trigger1 = statetype != A
trigger1 = ctrl
trigger2=movecontact
trigger2=stateno=200||stateno=201||stateno=210||stateno=211||stateno=230||stateno=231||stateno=235||stateno=240||stateno=241
trigger3=movecontact
trigger3=stateno=400||stateno=410||stateno=430||stateno=202
trigger4 = stateno = 250

;---------------------------------------------------------------------------------------------------------------------------
; ブルータル・アンギッシュＹ
[State -1]
type = ChangeState
value = 3405
triggerall = power >= 1000 
triggerall = command = "hado2_y" 
trigger1 = statetype != A
trigger1 = ctrl
trigger2=movecontact
trigger2=stateno=200||stateno=201||stateno=210||stateno=211||stateno=230||stateno=231||stateno=235||stateno=240||stateno=241
trigger3=movecontact
trigger3=stateno=400||stateno=410||stateno=430||stateno=202
trigger4 = stateno = 250

;---------------------------------------------------------------------------------------------------------------------------
; ライジングダークムーンＸ
[State -1]
type = ChangeState
value = 1020
triggerall = command = "syoryu_x" 
trigger1 = statetype != A
trigger1 = ctrl
trigger2=movecontact
trigger2=stateno=200||stateno=201||stateno=210||stateno=211||stateno=230||stateno=231||stateno=235||stateno=240||stateno=241
trigger3=movecontact
trigger3=stateno=400||stateno=410||stateno=430||stateno=202
trigger4 = stateno = 250

; ライジングダークムーンＹ
[State -1]
type = ChangeState
value = 1023
triggerall = command = "syoryu_y" 
trigger1 = statetype != A
trigger1 = ctrl
trigger2=movecontact
trigger2=stateno=200||stateno=201||stateno=210||stateno=211||stateno=230||stateno=231||stateno=235||stateno=240||stateno=241
trigger3=movecontact
trigger3=stateno=400||stateno=410||stateno=430||stateno=202
trigger4 = stateno = 250

;---------------------------------------------------------------------------------------------------------------------------
; テュホンレイジ・Ｘ
[State -1]
type = ChangeState
value = 1000
triggerall = command = "hado_x"
trigger1 = statetype != A
trigger1 = ctrl
trigger2=movecontact
trigger2=stateno=200||stateno=201||stateno=210||stateno=211||stateno=230||stateno=231||stateno=235||stateno=240||stateno=241
trigger3=movecontact
trigger3=stateno=400||stateno=410||stateno=430||stateno=202
trigger4 = stateno = 250

; テュホンレイジ・Ｙ
[State -1]
type = ChangeState
value = 1005
triggerall = command = "hado_y"
trigger1 = statetype != A
trigger1 = ctrl
trigger2=movecontact
trigger2=stateno=200||stateno=201||stateno=210||stateno=211||stateno=230||stateno=231||stateno=235||stateno=240||stateno=241
trigger3=movecontact
trigger3=stateno=400||stateno=410||stateno=430||stateno=202
trigger4 = stateno = 250

;---------------------------------------------------------------------------------------------------------------------------
; リーサルインパクト_Ａ
[State -1]
type = ChangeState
value = 1010
triggerall = command = "hado_a"
trigger1 = statetype != A
trigger1 = ctrl
trigger2=movecontact
trigger2=stateno=200||stateno=201||stateno=210||stateno=211||stateno=230||stateno=231||stateno=235||stateno=240||stateno=241
trigger3=movecontact
trigger3=stateno=400||stateno=410||stateno=430||stateno=202
trigger4 = stateno = 250

; リーサルインパクト_Ｂ
[State -1]
type = ChangeState
value = 1017
triggerall = command = "hado_b" 
trigger1 = statetype != A
trigger1 = ctrl
trigger2=movecontact
trigger2=stateno=200||stateno=201||stateno=210||stateno=211||stateno=230||stateno=231||stateno=235||stateno=240||stateno=241
trigger3=movecontact
trigger3=stateno=400||stateno=410||stateno=430||stateno=202
trigger4 = stateno = 250

;---------------------------------------------------------------------------------------------------------------------------
; デスペレイトモーメント
[State -1]
type = ChangeState
value = 1030
triggerall = command = "gyakuyoga_x" 
trigger1 = statetype != A
trigger1 = ctrl
trigger2=movecontact
trigger2=stateno=200||stateno=201||stateno=210||stateno=211||stateno=230||stateno=231||stateno=235||stateno=240||stateno=241
trigger3=movecontact
trigger3=stateno=400||stateno=410||stateno=430||stateno=202
trigger4 = stateno = 250

; デスペレイトモーメント
[State -1]
type = ChangeState
value = 1035
triggerall = command = "gyakuyoga_y" 
trigger1 = statetype != A
trigger1 = ctrl
trigger2=movecontact
trigger2=stateno=200||stateno=201||stateno=210||stateno=211||stateno=230||stateno=231||stateno=235||stateno=240||stateno=241
trigger3=movecontact
trigger3=stateno=400||stateno=410||stateno=430||stateno=202
trigger4 = stateno = 250

;---------------------------------------------------------------------------------------------------------------------------
; ○○○
[State -1]
type = ChangeState
value = 1045
triggerall = command = "tatumaki_ab" 
trigger1 = statetype != A
trigger1 = ctrl
trigger2=movecontact
trigger2=stateno=200||stateno=201||stateno=210||stateno=211||stateno=230||stateno=231||stateno=235||stateno=240||stateno=241
trigger3=movecontact
trigger3=stateno=400||stateno=410||stateno=430||stateno=202
trigger4 = stateno = 250

; ○○○
[State -1]
type = ChangeState
value = 1040
triggerall = command = "tatumaki_a" 
trigger1 = statetype != A
trigger1 = ctrl
trigger2=movecontact
trigger2=stateno=200||stateno=201||stateno=210||stateno=211||stateno=230||stateno=231||stateno=235||stateno=240||stateno=241
trigger3=movecontact
trigger3=stateno=400||stateno=410||stateno=430||stateno=202
trigger4 = stateno = 250

; ○○○
[State -1]
type = ChangeState
value = 1041
triggerall = command = "tatumaki_b" 
trigger1 = statetype != A
trigger1 = ctrl
trigger2=movecontact
trigger2=stateno=200||stateno=201||stateno=210||stateno=211||stateno=230||stateno=231||stateno=235||stateno=240||stateno=241
trigger3=movecontact
trigger3=stateno=400||stateno=410||stateno=430||stateno=202
trigger4 = stateno = 250

;---------------------------------------------------------------------------------------------------------------------------
; デモンランディングＡ
[State -1]
type = ChangeState
value = 1050
triggerall = Facing != enemynear,Facing
triggerall = command = "hado_a" 
trigger1 = statetype = A
trigger1 = ctrl
trigger2=movecontact
trigger2=stateno=600||stateno=610||stateno=630||stateno=631||stateno=640||stateno=650

; デモンランディングＡ
[State -1]
type = ChangeState
value = 1053
triggerall = Facing = enemynear,Facing
triggerall = command = "tatumaki_a" 
trigger1 = statetype = A
trigger1 = ctrl
trigger2=movecontact
trigger2=stateno=600||stateno=610||stateno=630||stateno=631||stateno=640||stateno=650

;---------------------------------------------------------------------------------------------------------------------------
; デモンランディングＢ
[State -1]
type = ChangeState
value = 1055
triggerall = Facing != enemynear,Facing
triggerall = command = "hado_b" 
trigger1 = statetype = A
trigger1 = ctrl
trigger2=movecontact
trigger2=stateno=600||stateno=610||stateno=630||stateno=631||stateno=640||stateno=650

; デモンランディングＢ
[State -1]
type = ChangeState
value = 1057
triggerall = Facing = enemynear,Facing
triggerall = command = "tatumaki_b" 
trigger1 = statetype = A
trigger1 = ctrl
trigger2=movecontact
trigger2=stateno=600||stateno=610||stateno=630||stateno=631||stateno=640||stateno=650

;---------------------------------------------------------------------------------------------------------------------------
; カットスピン
[State -1]
type = ChangeState
value = 202
triggerall = command = "fwd_x"
triggerall = command != "holddown"
trigger1 = statetype = S
trigger1 = ctrl
trigger2=movecontact
trigger2=stateno=200||stateno=201||stateno=210||stateno=211||stateno=230||stateno=231||stateno=235||stateno=240||stateno=241
trigger3=movecontact
trigger3=stateno=400||stateno=410||stateno=430
trigger4 = stateno = 250
trigger5 = stateno = 100

;---------------------------------------------------------------------------------------------------------------------------
;ふっとばし
[State -1]
type = ChangeState
value = 250
triggerall = command = "b+y"|| command = "c"
trigger1 = statetype = S
trigger1 = ctrl
trigger2 = stateno = 100

;--------------------------------------------------------------
; Ｐ＆Ｋ投げ
[State -1,Throw]
type = ChangeState
value = 800
triggerall = command = "y" || command = "b"
triggerall = statetype = S
triggerall = ctrl
triggerall = stateno != 100
trigger1 = command = "holdfwd"
trigger1 = p2bodydist X < 5
trigger1 = (p2statetype = S) || (p2statetype = C)
trigger1 = p2movetype != H
trigger2 = command = "holdback"
trigger2 = p2bodydist X < 5
trigger2 = (p2statetype = S) || (p2statetype = C)
trigger2 = p2movetype != H
trigger3 = stateno = 1092
trigger4 = stateno = 100

;--------------------------------------------------------------
;立ち弱パンチ
[State -1, Stand Light Punch]
type = ChangeState
value = 201
triggerall = command = "x"
triggerall = command != "holddown"
triggerall = P2BodyDist X <= 18
trigger1 = statetype = S
trigger1 = ctrl
trigger2 = stateno = 100

;立ち弱パンチ
[State -1, Stand Light Punch]
type = ChangeState
value = 200
triggerall = command = "x"
triggerall = command != "holddown"
trigger1 = statetype = S
trigger1 = ctrl
trigger2 = stateno = 400
trigger3 = StateNo = 200 && (time > 9) 
trigger4 = stateno = 100

;--------------------------------------------------------------
;立ち強パンチ
[State -1, Stand Strong Punch]
type = ChangeState
value = 211
triggerall = command = "y"
triggerall = command != "holddown"
triggerall = P2BodyDist X <= 18
trigger1 = statetype = S
trigger1 = ctrl
trigger2 = stateno = 100

;立ち強パンチ
[State -1, Stand Strong Punch]
type = ChangeState
value = 210
triggerall = command = "y"
triggerall = command != "holddown"
trigger1 = statetype = S
trigger1 = ctrl
trigger2 = stateno = 100

;--------------------------------------------------------------
;立ち弱キック
[State -1, Stand Light Kick]
type = ChangeState
value = 231
triggerall = command = "a"
triggerall = command != "holddown"
triggerall = P2BodyDist X <= 18
trigger1 = statetype = S
trigger1 = ctrl
trigger2 = stateno = 100

;立ち弱キック
[State -1, Stand Light Kick]
type = ChangeState
value = 230
triggerall = command = "a"
triggerall = command != "holddown"
trigger1 = statetype = S
trigger1 = ctrl
trigger2 = stateno = 100

;--------------------------------------------------------------
;立ち強キック
[State -1, Standing Strong Kick]
type = ChangeState
value = 241
triggerall = command = "b"
triggerall = command != "holddown"
triggerall = P2BodyDist X <= 18
trigger1 = statetype = S
trigger1 = ctrl
trigger2 = stateno = 100

;立ち強キック
[State -1, Standing Strong Kick]
type = ChangeState
value = 240
triggerall = command = "b"
triggerall = command != "holddown"
trigger1 = statetype = S
trigger1 = ctrl
trigger2 = stateno = 100

;---------------------------------------------------------------------------
;挑発
[State -1, Taunt]
type = ChangeState
value = 196
triggerall = command = "6s"
trigger1 = statetype != A
trigger1 = ctrl

;挑発
[State -1, Taunt]
type = ChangeState
value = 195
triggerall = command = "start"
trigger1 = statetype != A
trigger1 = ctrl

;--------------------------------------------------------------
;しゃがみ弱パンチ
[State -1, Crouching Light Punch]
type = ChangeState
value = 400
triggerall = command = "x"
triggerall = command = "holddown"
trigger1 = statetype = C
trigger1 = ctrl
trigger2 = (stateno = 400) || (stateno = 430)
trigger2 = (time > 9) || (movecontact && time > 5)
trigger3 = stateno = 100

;--------------------------------------------------------------
;しゃがみ強パンチ
[State -1, Crouching Strong Punch]
type = ChangeState
value = 410
triggerall = command = "y"
triggerall = command = "holddown"
trigger1 = statetype = C
trigger1 = ctrl
trigger2 = stateno = 100

;--------------------------------------------------------------
;しゃがみ弱キック
[State -1, Crouching Light Kick]
type = ChangeState
value = 430
triggerall = command = "a"
triggerall = command = "holddown"
trigger1 = statetype = C
trigger1 = ctrl
trigger2 = (stateno = 400) || (stateno = 430)
trigger2 = (time > 9) || (movecontact && time > 5)
trigger3 = stateno = 100

;--------------------------------------------------------------
;しゃがみ強キック
[State -1, Crouching Strong Kick]
type = ChangeState
value = 440
triggerall = command = "b"
triggerall = command = "holddown"
trigger1 = statetype = C
trigger1 = ctrl
trigger2 = stateno = 100

;--------------------------------------------------------------
;空中ふっとばし
[State -1]
type = ChangeState
value = 650
triggerall = command = "b+y"|| command = "c"
trigger1 = statetype = A
trigger1 = ctrl
trigger2 = (stateno = 1091) && Var(9) = 1

;--------------------------------------------------------------
;空中弱パンチ
[State -1, Jump Light Punch]
type = ChangeState
value = 600
triggerall = command = "x" && statetype = A
trigger1 = ctrl
trigger2 = (stateno = 1091) && Var(9) = 1

;--------------------------------------------------------------
;空中強パンチ
[State -1, Jump Strong Punch]
type = ChangeState
value = 610
triggerall = command = "y" && statetype = A
trigger1 = ctrl
trigger2 = (stateno = 1091) && Var(9) = 1

;--------------------------------------------------------------
;空弱Ｋ
[State -1, Jump Light Kick]
type = ChangeState
value = 630
triggerall = command = "a" && vel x  && statetype = A 
trigger1 = ctrl

;空弱Ｋ↑
[State -1, Jump Light Kick]
type = ChangeState
value = 631
triggerall = command = "a" && statetype = A
trigger1 = ctrl

;--------------------------------------------------------------
;空中強キック
[State -1, Jump Strong Kick]
type = ChangeState
value = 640
triggerall = command = "b" && statetype = A
trigger1 = ctrl




